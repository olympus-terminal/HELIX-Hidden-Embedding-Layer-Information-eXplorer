...
def read_sequences_from_file(file_path, max_sequences=10):
    ...
    with open(file_path, 'r') as file:
        return [line.strip() for line in file if line.strip()][:max_sequences]
...
def plot_sequence_layers(hidden_states, sequence, ax, sequence_name=""):
    ...
    markers = ['o', 's', '^', 'D', 'v', '<', '>', 'p', 'h', '8', '*', 'H', '+', 'x', 'd']
    aa_to_color = dict(zip(unique_aa, colors))
    aa_to_marker = dict(zip(unique_aa, markers[:len(unique_aa)]))
...
def process_sequences(file_paths, max_sequences=10):
    ...
    # Process each file
    with PdfPages('sequence_representations_a4.pdf') as pdf:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PRISM: Protein Representation In Structure Mapping")
    ...
    process_sequences(args.file_paths, args.max_sequences)
